"""A module containing the DataManager class, which handles the generation and labelling
of training and validation data for the BFBLearner class.
"""
...
class labeller_wrapper:
    ...
    ----------
    func : callable
        A numeric function for the potential. This is a numeric function generated by the class DataManager in its init method. It will take numeric arrays (of a format depending on the DataManager class) representing
        a scalar vev and a set of quartic potential coefficients and return the numerical value of the quartic part of the potential function and its gradient with respect to the vev.
    ...
    polar : bool
        If True, then the analysis of the potential will be conducted with a polar coordinate parameterization of the vev parameters. 
        If False, then Cartesian coordinates will be used, albeit with the vev parameters restricted to a phi_len-dimensional unit hypersphere.

    label_fn : callable, optional.
        A function that takes a 2-D NumPy array of quartic coefficients and returns a list of Boolean labels for them. This is for implementing customized oracle functions.
        Must have the signature (func: Callable, phi_len: int, polar: bool, rng: NumpyGenerator, lam: np.array(np.float, np.float), **kwargs) -> np.array(bool)
        If this argument is not specified, the default oracle BFBrain.Jax_Oracle.label_func is used.

    label_check : callable, optional
        A function that can be used to test the reliability of a custom oracle given by label_fn, or if label_fn is None, the default oracle BFBrain.Jax_Oracle.label_func.
        Must have the same signature as label_fn, up to additional keyword arguments.
        If this argument is not specified, the a tester for the default oracle is used: BFBrain.Jax_Oracle.test_labeller

    **label_kwargs : dict, default=dict(niter = 250, tol = 0.001, cutoff = 150000)
    ...
    """
    def __init__(self, func, phi_len, lam_len, rng, polar, label_fn = None, label_check = None, label_kwargs = dict(niter = 250, tol = 0.001, cutoff = 150000)):
        self.func = func
    ...
    def do_labelling(self, lam, label_kwargs = None):
        ...
        label_kwargs : dict, optional
            An optional alternative set of oracle keyword arguments. If not specified, the class instance's label_kwargs
            attribute is used.
    ...
    def check_labeller(self, lam, **tester_kwargs):
        """Tests the reliability of the labelling-- calls self.label_check. Depending on the methodology of label_func, this function may or may not be useful.
        For example, a rigorous computation of boundedness-from-below based on resultants would not require any consistency or reliability checks.
...
class np_data:
    ...
    def from_file(cls, path):
        """A constructor for loading an np_data object from a .npz file (see NumPy documentation), likely created in a previous run of this program.

        Parameters
        ----------
        path : str
            A string with a file name. '.npz' is appended to the end of the string, and should not be included in path.
    ...
    def save_data(self, path):
        ...
        ----------
        path : str
            A string with a file name. If .npz is not at the end of the string, it is appended to it.
        """
...
class DataManager:
    ...
    Note that this class contains all the random number generation that's not specifically associated with the neural network and its optimizer.
    Generally one should use the 'from_seed' or 'from_file' constructor rather than constructing from the base initialization method.
    ...
    rng : list of numpy.random.Generator
        A list of NumPy random number generators which control all the random generation related to the generation and labelling of data. 
        In total there are 6 random number generators, each differently seeded using NumPy's SeedSequence.spawn method. 
        Each random number generator is used only for one specific task: Generating training data, generating validation data, generating random points
        in the vicinity of other points (two rng's are used here, one for rotation direction and the other for rotation angle), 
        doing random number generation associated with labelling, and shuffling data for training.
    ...
    lambdify_mode : {'jax', 'numpy', 'scipy', 'math', 'mpmath', 'numexpr', 'sympy', 'tensorflow'}
        Passed directly as the argument "modules" in sympy.lambdify, the function used to generate numerical functions from the symbolic expression for the scalar potential.
        Default value is 'jax', consistent with the default oracle function, BFBrain.Jax_Oracle.label_func
    ...
    **label_kwargs : dict, optional
    ...
    def __init__(self, phi_len, lam_len, rng, polar, sym_expr, sym_grad_expr, phisym_var, lamsym, lambdify_mode = 'jax', label_fn = None, label_check = None, **label_kwargs):
        ...
        #Now create numerical functions for the value and gradient of the quartic part of the potential from the symbolic function:
    ...
    def from_func(cls, sym_func, phi_len, lam_len, seed = None, polar = False, lambdify_mode = 'jax', label_fn = None, label_check = None, **label_kwargs):
        ...
        sym_func : SymPy function.
            A SymPy function that expresses the quartic part of the potential. Must have the signature (sympy.Array, sympy.Array) -> sympy.Expr, where the first sympy.Array object corresponds to the vev configuration
            and the second corresponds to the quartic coefficients in the potential.

        phi_len : int
            The number of real parameters needed to uniquely specify the vev in the model.

        lam_len : int
            The number of independent real quartic coupling coefficients in the model's potential function.

        seed : int, optional
            A random number seed. Used to spawn a sequence of random generators with SeedSequence.

        polar : bool, default=False
            If true, the potential is analyzed with the vev coordinates converted to a polar form. If false, they are analyzed in their Cartesian form.

        lambdify_mode : {'jax', 'numpy', 'scipy', 'math', 'mpmath', 'numexpr', 'sympy', 'tensorflow'}
            The "module" input to sympy.lambdify, used to extract numerical expressions from the symbolic SymPy function. See SymPy documentation for details.
    ...
    def __getstate__(self):
        """Modify pickle's saving of this class to avoid unpicklable objects.
        """
    ...
    def __setstate__(self, state):
        """Modify pickle's loading of this class to reconstruct unpicklable objects.
        """
    ...
    def create_random_lambdas(self, nlams, validation = False):
        """Create a list of random sets of quartic potential coefficients (but don't label them yet). Use independent uncorrelated rng's for the generation of a validation and training set.
        Notice that these lambdas are Cartesian coordinates that uniformly sample the unit hypersphere.

        Parameters
        ----------
        nlams : int
            The number of sets of quartic coefficients to generate randomly.

        validation: bool, default=False
            A flag denoting which random number generator to use, ensuring independently-generated validation and training sets. If True, use the random number generator for the validation set,
            while if False, use the random number generator for the training set.
    ...
    def check_labeller(self, nlams, **tester_kwargs):
        ...
        ----------
        nlams : int
            The number of sets of quartic coefficients to randomly generate for testing the labeller function's consistency.

        tester_kwargs : dict
            Additional keyword arguments required by the check_labeller function. If the default oracle and tester are used, possible keyword arguments are niter_step, count_success, max_iter, verbose.
            See BFBrain.Jax_Oracle.test_labeller for details.

        Returns
        -------
            Same type as labeller.check_labeller, which may be a user-written function. If the default oracle and tester are used, this function will be BFBrain.Jax_Oracle.test_labeller.
        """
    ...
    def checklam_all(self, lams, truth_label_fn = None, label_kwargs = None):
        ...
        lams : np.array(np.float32, np.float32)
            A 2-D NumPy array of quartic coefficients of the potential. Each entry along the 0 axis corresponds to a single set of quartic potential coefficients specifying a potential function.
        ...
        truth_label_fn : callable, optional
            ...
            If this argument is specified, the method will use this callable to label lams instead of the labeller class. This is used in specific instances when a fast symbolic expression
        ...
        label_kwargs : dict, optional
            If these are specified, the oracle will use the keyword arguments given here instead of the keyword arguments specified in the DataManager constructor.
    ...
    def create_random_data(self, nlams, validation = False, truth_label_fn = None, label_kwargs = None):
        ...
        ----------
        nlams : int
            The number of sets of quartic potential coefficients to generate.

        validation : bool, default=False
            If True, use the validation random number generator to generate the random coefficients. If False, use the training random number generator.
    ...
    def check_accuracy_with_better_labeller(self, in_data, label_kwargs):
        """A method for evaluating the accuracy of a labeller which is capable of mislabelling some False points as True,
        like the default oracle, which is based on global minimization of the quartic part of the potential.

        Parameters
        ----------
        in_data : np_data
            An np_data object, labelled with an oracle that can mislabel some False points as True (but not the reverse).

        label_kwargs : dict
            A dictionary which specifies the keyword arguments for the oracle, which must be selected to yield significantly more accurate labels than the ones specified by in_data.
    ...
    def balance_array(self, data):
        """Given an np_data object that has more negative (not bounded-from-below) points than positives (bounded-from-below), rebalance data to include new positive points generated by leveraging
        the convexity of the space of bounded-from-below points. If there are as many or more positive points as negative points in the np_data object, leaves the np_data object unmodified.

        Parameters
        ----------
        data : np_data
            The np_data object that has many more negatively-labelled points (that is, points which are not bounded-from-below) than positives.
        """
        ...
        # If there are no members of one label, or there are an exactly equal number of members of both labels, or there are already more positive points than negative points, do nothing.
        if(pos == 0 or neg == 0 or pos == neg or pos > neg):
            return
        # Generate new points from the positive points that are already in the data.
    ...
    def _create_new_positives(self, lams, nlams):
        """Generates new bounded-from-below points from existing known ones. Called by balance_array.
        ...
        -------
        np.array(np.float32, np.float32)
            A 2-D NumPy array representing nlams sets of new quartic coefficients which are bounded-from-below, since they are sampled along line segments between existing bounded-from-below points.
        """
    ...
    def create_dataset(self, data, validation = False):
        ...
        # Create NumPy arrays of the quartic coefficient sets (lams) and their labels (labs).
        ...
        if not validation:
            ...
            lams = lams[shuff]
            labs = labs[shuff]
        # Return the tf.data.Dataset object created from the coefficient sets and their labels.
        return tf.data.Dataset.from_tensor_slices((lams, labs))

    def generate_L(self, nL, lams, hop_dist, probs = None, rand_fraction = 0.):
        """Randomly generate a sample of new points in the vicinity of some existing points. Given some set of existing points, samples new points by making
        random hops of an angle given by a draw from a normal distribution in a random direction along the unit hypersphere in quartic coefficient space.
        ...
        lams : np.array(np.float32, np.float32)
            A 2-D NumPy array representing sets of quartic potential coefficients. New points will be sampled in the vicinity of these.

        hop_dist : float
            The distance scale for sampling around the coefficients in lams. Newly-generated points are taken from input points by randomly rotating points in lams
            by an angle taken from a normal distribution with standard deviation hop_dist.

        probs : np.array(np.float32), optional
            A 1-D NumPy array, should be an array of nonnegative floats which sum to 1, representing the probability of the function selecting each index of lams to generate new
            points around. If not specified, a uniform selection probability for all points in lams is used.

        rand_fraction : float, optional
            Must be a non-negative float between 0 and 1. If specified, then the method will sample that fraction of its points as uniformly distributed draws from the surface
            of the unit hypersphere in quartic coefficient space, instead of sampling in the vicinity of points in lams. If not specified, all generated points will be sampled in the
            vicinity of points in lams.

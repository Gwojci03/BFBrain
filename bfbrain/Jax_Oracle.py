"""The code in this module contains BFBrain's default algorithm for labelling if a point is bounded from below. 
It is designed to work with the methods in Active_Learning.py. The fundamental strategy of this labelling method 
is to take a quartic part of the potential function with fixed quartic coefficients, and attempt many consecutive 
local optimizations with respect to the scalar vev with random initial starting points. If it can find a point 
where the potential is negative, it will label the set of quartic coefficients as not bounded from below, while 
if after some user-specified number of local minimization iterations the found minima are always positive, the 
set will be labelled as bounded from below.
"""
...
def take_step(x0, key, polar):
    ...
    x0 : jnp.array(jnp.float32)
        A Jax NumPy array that tells the function what shape the vev input arrays should be.

    key : Jax PRNGGKey
        A key object used to generate random numbers in Jax. Will be consumed over the running of the function and a new key will be returned.

    polar : bool
        A flag denoting whether the labeller should use Cartesian coordinates for the vev (False) or convert them into polar coordinates on the unit hypersphere (True).
    ...
    new_key : Jax PRNGKey
        A key object used to generate random numbers in Jax. Supplied to replace the consumed key.
    """
    # Generate two new keys from key: subkey to be consumed to generate a random vev configuration and new_key to pass back for future random number generation.
    ...
    # If the minimizer is using polar coordinates, convert rand_phi to polar coordinates. Otherwise simply return rand_phi (and new_key).
    if(polar):
...
def run_one_step(key, minimizer, stepper, lam, polar):
    ...
    stepper : callable
        The function take_step, wrapped to require only a PRNGKey object as input.

    lam : jnp.array(jnp.float32)
        A 1-D Jax Numpy array representing a set of quartic coefficients in the scalar potential.

    Returns
    -------
    energy_after_quench : jnp.float32
        The minimum value of the potential found by minimizer after starting from a random starting position generated with key.

    new_key : Jax PRNGKey
        A key object used to generate random numbers in Jax. Supplied to replace the consumed key.
    """
    # Generate an initial position for the minimizer to use from the function stepper. This will also consume the PRNG key and necessitate acquiring a new one.
    x_after_step, new_key = stepper(key = key)
    # Run the minimizer from the point x_after_step. The arguments for the bounds on variables in the minimizer depend on whether the minimizer is using polar coordinates or not.
    if(polar):
    ...
    # Compute the value of the quartic part of the potential at the minimum.
    # energy_after_quench = jnp.minimum(minimizer.fun(minres[0] - minimizer.fun(minres[0], lam)[1], lam)[0], minimizer.fun(minres[0], lam)[0])
...
def jax_basinhopping(lam, rng_key, x0, minimizer, niter, polar):
    ...
    niter : int
        The maximum number of run_one_step iterations to perform in an attempt to find a negative local minimum of the potential.
    ...
    new_min_energy: jnp.float32
        The smallest local minimum that the optimizer found after its iterations.
    """
    ...
    # Define a Jax while loop. This loop performs run_one_step repeatedly and keeps track of the minimum potential value it's found as min_energy. If niter iterations have been performed or min_energy is
...
def vectorized_minTest(func, lam, key, x0, niter, tol, polar):
    ...
    """
    # Print statement to alert user to retracing. Retracing is expensive and should be minimized.
    print('recompiling vectorized_minTest...')
    # Set up a local minimizer based on projected gradient descent, with slightly different construction depending on whether or not the solver is using polar coordinates.
    if(polar):
...
def label_func(func, phi_len, polar, rng, lam, niter = 100, tol = 0.001, cutoff = 150000):
    ...
    ----------
    func : callable
        The Jax Numpy function that returns the quartic part of the potential and the gradient.
    ...
    rng : np.random.Generator
        The NumPy random number generator which will generate the initial PRNGKey used by this oracle.

    lam : np.array(np.float32,np.float32)
        A 2-D Numpy array of quartic potential coefficients.

    niter : int, default=100
        The number of local minimizations to perform on the potential before declaring it to be bounded-from-below.

    tol : float, default=0.001
        The tolerance for the local minimizer.

    cutoff : int, default=150000
        The maximum size of a batch of coefficient values to pass to the GPU at one time. If lam consists of more sets of coefficient values than this, the method will split it into digestible batches.
    ...
    """
    # If the size of lam is below cutoff, transfer the entirety of lam to the GPU at once and label it.
    if(len(lam) <= cutoff):
        return label_func_do_batch(func, phi_len, polar, rng, lam, niter, tol)
    # Otherwise, split lam into smaller chunks that are guaranteed to have length smaller than cutoff.
    else:
...
def test_labeller(func, phi_len, polar, rng, lam, niter = 100, tol = 0.001, cutoff = 150000, niter_step = 50, count_success = 5, max_iter = 20, verbose = False):
    """A method to test the accuracy of the oracle. Will perform label_func repeatedly for the same 2-D NumPy array of quartic coefficients, but with niter increased each time,
    until the same labels are returned for for a specified consecutive number of iterations, or some maximum number of labellings has been completed without finding consistent results.
    ...
    niter : int, default=100
        The initial number of local minimizations to perform on the potential before declaring it to be bounded-from-below-- this value will be incremented over the running of the method.
    ...
    niter_step : int, default=50
        The amount to increment the niter parameter of label_func with each successive attempt at labelling lam.

    count_success : int, default=5
        The number of consecutive labelling attempts that must yield identical labels for the function to declare that increasing niter is no longer affecting the results of label_func.

    max_iter : int, default=20
        The maximum number of labelling attempts that the method will make. If no consistent results are found before that time, the test ends in failure.

    verbose : bool, default=False
        If True, print out statements informing the user of the progress of the method.
        
    Returns
    -------
    int
        The minimum niter parameter such that count_success consecutive attempts to label lam with increasing niter yielded the same label. 
        If max_iter attempts are made without running into count_success consecutive identical label results, -1 is returned.
    """
    # An integer to keep track of the number of consecutive iterations with identical labels.
    count = 0
    # A NumPy array to keep track of the latest iteration's labels.
    current_res = np.zeros(shape=len(lam), dtype=bool)
    # The index of the earliest iteration to give the same labels as the current iteration.
    current_ind = 0
    for i in range(max_iter):
        # In the loop, repeatedly call label_func. With each loop iteration, increment niter by niter_step. 
        if(verbose):
            print('doing round ' + str(i))
        new_res = label_func(func, phi_len, polar, rng, lam, niter + i*niter_step, tol, cutoff)
        if(verbose):
            print('done!')
        # If the labels produced from this iteration differ at all from those produced by the previous iteration, reset count and update current_res, and current_ind.
        ...
        if(is_update):
            ...
            current_res = new_res
            current_ind = i
        # If the labels are the same as the previous iteration, increment count.
        else:
            count += 1
        # If count_success consecutive iterations have given the same labels, or it is now impossible for that to occur before max_iter iterations are completed, break out of the loop.
        if(count >= count_success or count + max_iter - i - 1 < count_success):
            break
    # If the function was successful in finding consistent labels, return the smallest niter value such that consecutive iterations yielded the same result. If the function was unsuccessful, return -1.
